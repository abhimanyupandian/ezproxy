...
function ruleForRemoteStop(req) {
  if(req.url.includes("stop_proxy")) {
    const { username, password } = req.requestData.toString();
...
class ProxyServer {
  constructor(port, networkSettings) {
    ...
    this.enableRemoteAccessServices(networkSettings);
  }

  enableRemoteAccessServices(networkSettings) {
    
    var _remoteAccess = networkSettings.allowRemoteAccess || false;
    if (_remoteAccess) {
      const _username = networkSettings.credentials.username || null;
      const _password = networkSettings.credentials.password || null;
      if(!_username) {
        console.error("Username not provided for Remote Access. Remote access will be disabled!");
      } else if (!_password) {
          console.error("Password not provided for Remote Access. Remote access will be disabled!");
      } else {
        this.proxyServer.requestHandler._username = _username;
        this.proxyServer.requestHandler._password = _password;

        this.addRuleOnRequest("_stop", ruleForRemoteStop)
